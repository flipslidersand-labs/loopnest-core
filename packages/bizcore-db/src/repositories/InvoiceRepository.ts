  ...
  async findAll(filter: InvoiceFilter = {}): Promise<InvoiceRecord[]> {
    if (filter.skip !== undefined) {
      process.stderr.write(
        "[InvoiceRepository] skip/take is deprecated; use cursor-based pagination\n",
      );
    }
    let q = this.db
    ...
    if (filter.createdAtFrom)
      q = q.where("created_at", ">=", new Date(filter.createdAtFrom));
    if (filter.createdAtTo)
      q = q.where("created_at", "<", new Date(filter.createdAtTo));
  ...
  private map(r: any): InvoiceRecord {
    return {
      ...
      discountAmount: r.discount_amount
        ? parseFloat(r.discount_amount.toString())
        : 0,
      ...
      exchangeRate: r.exchange_rate
        ? parseFloat(r.exchange_rate.toString())
        : 1.0,
    };
  }
}
