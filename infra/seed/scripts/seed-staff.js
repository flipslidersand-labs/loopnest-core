...
async function main() {
  try {
    ...
    const orgYaml = yaml.load(
      fs.readFileSync(
        path.join(__dirname, "../static/organizations.yaml"),
        "utf-8",
      ),
    );
    ...
    for (const staff of staffYaml.staff) {
      ...
      if (!org_id) {
        console.warn(
          `⚠️ Organization not found for ${staff.name} (org: ${staff.organization_id}), skipping`,
        );
        continue;
      }
      ...
      const profile = {
        ...
        ...(staff.customer_segment && {
          customer_segment: staff.customer_segment,
        }),
      };
