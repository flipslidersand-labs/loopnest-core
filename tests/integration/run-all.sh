...
if [ "$#" -gt 0 ]; then
  ...
  SUITES=(observability auth tenancy e2e_workflow quote_items invoices pdf_invoice payments credit_notes audit_api reports webhooks outbox_dispatch approvals error_scenarios concurrency idempotency rate_limit search members)
fi
