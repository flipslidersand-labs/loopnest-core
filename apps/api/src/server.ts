...
import {
  creditNoteRoutes,
  invoiceCreditNoteRoutes,
} from "./routes/creditNotes.js";
...
initializeDatabaseServices().then((dbServices: DatabaseServices) => {
  ...
  setKyselyQueryObserver((kind: string, ms: number) =>
    dbQueryDurationMs.observe(ms, { kind }),
  );
  ...
  app.use(
    "/api/customers",
    customerRoutes(dbServices.repos, serviceContainer.audit),
  );
  ...
  app.use(
    "/api/invoices",
    invoiceRoutes(dbServices.repos, serviceContainer.invoices),
  );
  ...
  app.use(
    ...
    rateLimit({
      bucket: "payments",
      windowSeconds: RATE_WINDOW_SECONDS,
      max: WORKFLOW_RATE_MAX,
    }),
    idempotencyMiddleware,
    invoicePaymentRoutes(
      serviceContainer.payments,
      dbServices.repos,
      serviceContainer.webhooks,
  ...
  app.use(
    ...
    recurringContractRoutes(dbServices.repos, serviceContainer.webhooks),
  );
  ...
  app.use(
    ...
    invoiceDunningRoutes(dbServices.repos),
  );
