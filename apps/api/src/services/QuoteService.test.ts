...
function makeRepos(overrides: Record<string, any> = {}) {
  return {
    quotes: {
      transitionStatus: vi
        .fn()
        .mockResolvedValue(makeQuote({ status: "pending_approval" })),
...
describe("QuoteService — submitForApproval", () => {
  ...
  it("throws 409 when transition is rejected (wrong status)", async () => {
    ...
    const svc = new QuoteService(repos as any);
    await expect(svc.submitForApproval("q-1", "user-1")).rejects.toThrow(
      ApiErrorResponse,
    );
  });
  ...
  it("throws 404 when quote does not exist", async () => {
    ...
    const svc = new QuoteService(repos as any);
    await expect(svc.submitForApproval("no-such", "user-1")).rejects.toThrow(
      ApiErrorResponse,
    );
  });
});
...
describe("QuoteService — approve", () => {
  ...
  it("passes notes to transition", async () => {
    ...
    expect((repos.quotes.transitionStatus as any).mock.calls[0][3]).toEqual({
      notes: "LGTM",
    });
  });
});
  ...
  it("transitions pending_approval → rejected with reason", async () => {
    const repos = makeRepos({
      quotes: {
        transitionStatus: vi
          .fn()
          .mockResolvedValue(makeQuote({ status: "rejected" })),
        findById: vi.fn(),
        findByStatus: vi.fn(),
      },
    });
...
describe("QuoteService — getWorkflowStatus", () => {
  ...
  it("throws 404 for unknown quote", async () => {
    ...
    const svc = new QuoteService(repos as any);
    await expect(svc.getWorkflowStatus("no-such")).rejects.toThrow(
      ApiErrorResponse,
    );
  });
});
...
describe("QuoteService — list helpers", () => {
  it("getDraftQuotes delegates to findByStatus with limit", async () => {
    ...
    expect(repos.quotes.findByStatus as any).toHaveBeenCalledWith("draft", {
      take: 5,
    });
  });
  ...
  it("getPendingApprovalQuotes uses correct status", async () => {
    ...
    expect(repos.quotes.findByStatus as any).toHaveBeenCalledWith(
      "pending_approval",
      { take: 10 },
    );
  });
