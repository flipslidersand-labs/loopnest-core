...
export class PdfService {
  ...
  async generateInvoicePdf(invoiceId: string): Promise<Buffer> {
    ...
    const data: DocumentData = {
      ...
      registrationNumber: (
        invoice as unknown as { registrationNumber?: string }
      ).registrationNumber,
      ...
      statusBadge:
        invoice.status === "paid" ? { text: "PAID", color: "green" } : null,
  ...
  async generateQuotePdf(
    ...
    const productIds = [
      ...new Set(
        (quote.items ?? []).map((i: { productId: string }) => i.productId),
      ),
    ];
    const products = await Promise.all(
      productIds.map((id: string) => this.repos.products.findById(id)),
    );
  ...
  private renderStatement(
    ...
    headers.forEach((h, i) =>
      doc.text(h, colX[i], doc.y, {
        width: colX[i + 1] ? colX[i + 1] - colX[i] - 5 : 80,
  ...
  private renderDocument(
    ...
    doc.text("数量", startX + colW.name, headerY, {
      width: colW.qty,
      align: "right",
    });
    doc.text("単価", startX + colW.name + colW.qty, headerY, {
      width: colW.unit,
      align: "right",
    });
    doc.text("金額", startX + colW.name + colW.qty + colW.unit, headerY, {
      width: colW.total,
      align: "right",
    });
    ...
    for (const item of data.items) {
      ...
      doc.text(String(item.quantity), startX + colW.name, rowY, {
        width: colW.qty,
        align: "right",
      });
      doc.text(
        formatCurrency(item.unitPrice),
        startX + colW.name + colW.qty,
        rowY,
        { width: colW.unit, align: "right" },
      );
      doc.text(
        formatCurrency(item.lineTotal),
        startX + colW.name + colW.qty + colW.unit,
        rowY,
    ...
    doc
      ...
      .lineTo(totalsX + totalsLabelW + totalsValW, doc.y)
      .stroke();
    doc.moveDown(0.3);
