...
describe("InvoiceService — pure calculations", () => {
  ...
  describe("calculateSubtotal", () => {
    ...
    it("handles fractional unit prices", () => {
      expect(
        svc.calculateSubtotal([{ quantity: 3, unitPrice: 33.33 }]),
      ).toBeCloseTo(99.99);
    });
  });
...
describe("InvoiceService — createFromQuote", () => {
  it("rejects non-approved quote", async () => {
    const repos = makeRepos({
      quotes: {
        findWithItems: vi
          .fn()
          .mockResolvedValue(makeQuote({ status: "draft" })),
      },
    });
    ...
    await expect(svc.createFromQuote("quote-1", "user-1")).rejects.toThrow(
      ApiErrorResponse,
    );
  });
  ...
  it("accepts invoiced status (re-create guard)", async () => {
    const repos = makeRepos({
      quotes: {
        findWithItems: vi
          .fn()
          .mockResolvedValue(makeQuote({ status: "invoiced" })),
      },
    });
    const svc = new InvoiceService(
      repos as any,
    ...
    await expect(
      svc.createFromQuote("quote-1", "user-1"),
  ...
  it("throws 404 when quote not found", async () => {
    ...
    await expect(svc.createFromQuote("no-such", "user-1")).rejects.toThrow(
      ApiErrorResponse,
    );
  });
  ...
  it("skips credit check for unlimited customers", async () => {
    ...
    await expect(
      svc.createFromQuote("quote-1", "user-1"),
    ).resolves.toMatchObject({ invoiceId: "inv-1" });
  });
  ...
  it("publishes invoice_created outbox event", async () => {
    ...
    expect((repos.outbox.publish as any).mock.calls[0][0]).toBe(
      "invoice_created",
    );
  });
  ...
  it("increments customer credit usage on success", async () => {
    ...
    await svc.createFromQuote("quote-1", "user-1");
    expect(repos.customers.incrementCreditUsed as any).toHaveBeenCalledWith(
      "cust-1",
      expect.any(Number),
    );
  });
  ...
  it("applies discount to taxable amount", async () => {
    ...
    expect((repos.invoices.create as any).mock.calls[0][0].totalAmount).toBe(
      880,
    );
