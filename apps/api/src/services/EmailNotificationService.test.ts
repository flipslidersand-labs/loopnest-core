  ...
  return {
    invoices: {
      findById: vi
        .fn()
        .mockResolvedValue(
          invoiceOverride !== undefined ? invoiceOverride : makeInvoice(),
        ),
    },
    customers: {
      findById: vi
        .fn()
        .mockResolvedValue(
          customerOverride !== undefined ? customerOverride : makeCustomer(),
        ),
    },
  };
}
...
describe("EmailNotificationService — sendInvoiceIssued", () => {
  it("sends email and increments counter", async () => {
    ...
    expect(mockSendMail.mock.calls[0][0].subject).toContain(
      "INV-202601-000001",
    );
  ...
  it("skips when customer has no email", async () => {
    const svc = new EmailNotificationService(
      makeRepos(undefined, makeCustomer({ email: undefined })) as any,
    );
    await svc.sendInvoiceIssued("inv-1");
    expect(mockSendMail).not.toHaveBeenCalled();
  });
...
describe("EmailNotificationService — sendPaymentReminder", () => {
  ...
  it("skips when invoice is already paid", async () => {
    const svc = new EmailNotificationService(
      makeRepos(makeInvoice({ status: "paid" })) as any,
    );
    await svc.sendPaymentReminder("inv-1");
    expect(mockSendMail).not.toHaveBeenCalled();
  });
