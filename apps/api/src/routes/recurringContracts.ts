...
export function recurringContractRoutes(
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const contracts = await repos.recurringContracts.findAll({
        customerId,
        status: status as
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (!customerId || !name) {
        throw new ApiErrorResponse(
          400,
          "VALIDATION_ERROR",
          "customerId and name are required",
        );
      }
      if (!["day", "week", "month", "year"].includes(intervalUnit)) {
        throw new ApiErrorResponse(
          400,
          "VALIDATION_ERROR",
          "intervalUnit must be day, week, month, or year",
        );
      }
      ...
      const contract = await repos.recurringContracts.create({
        ...
        intervalValue: iv,
        amount: amt,
        taxRate: taxRate != null ? Number(taxRate) : 0.1,
        startsAt,
        endsAt,
        lineItems,
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (contract.status !== "active") {
        throw new ApiErrorResponse(
          409,
          "CONFLICT",
          `Contract is ${contract.status}, cannot pause`,
        );
      }
      ...
      if (pauseUntil && isNaN(Date.parse(pauseUntil))) {
        throw new ApiErrorResponse(
          400,
          "VALIDATION_ERROR",
          "pauseUntil must be a valid ISO date",
        );
      }
      const updated = await repos.recurringContracts.pause(
        req.params.id,
        reason,
        pauseUntil ?? null,
      );
      wh?.deliver(req.user?.orgId, "contract.paused", {
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (contract.status !== "paused") {
        throw new ApiErrorResponse(
          409,
          "CONFLICT",
          `Contract is ${contract.status}, cannot resume`,
        );
      }
      const updated = await repos.recurringContracts.resume(req.params.id);
      wh?.deliver(req.user?.orgId, "contract.resumed", {
        contractId: req.params.id,
      });
      res.json({ data: updated });
    }),
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (contract.status === "cancelled" || contract.status === "completed") {
        throw new ApiErrorResponse(
          409,
          "INVALID_STATUS",
          `Contract is already ${contract.status}`,
        );
      }
      const updated = await repos.recurringContracts.updateStatus(
        req.params.id,
        "cancelled",
      );
      res.json({ data: updated });
