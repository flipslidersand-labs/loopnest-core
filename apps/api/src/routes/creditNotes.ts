...
import { RepositoryContainer, CreditNoteType, CreditNoteStatus } from '@loopnest/bizcore-db';
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (amount === undefined || !reason) {
        throw new ApiErrorResponse(400, 'VALIDATION_ERROR', 'amount and reason are required');
      }
...
export function creditNoteRoutes(
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (!targetInvoiceId || amount === undefined) {
        throw new ApiErrorResponse(400, 'VALIDATION_ERROR', 'targetInvoiceId and amount are required');
      }
      ...
      res.status(201).json({ data: result.application, balance: result.creditNoteBalance });
    })
  );
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      res.json({ data: result.creditNote, refundedAmount: result.refundedAmount });
    })
  );
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      webhooks.deliver(req.user?.orgId, 'credit_note.voided', { creditNoteId: id });
