    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (!rate)
        throw new ApiErrorResponse(
          404,
          "NOT_FOUND",
          `Exchange rate not found for: ${req.params.code}`,
        );
      res.json({ data: rate });
    }),
  );
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (!currencyCode || rateToJpy === undefined || !effectiveDate) {
        throw new ApiErrorResponse(
          400,
          "VALIDATION_ERROR",
          "currencyCode, rateToJpy, and effectiveDate are required",
        );
      }
      if (typeof rateToJpy !== "number" || rateToJpy <= 0) {
        throw new ApiErrorResponse(
          400,
          "VALIDATION_ERROR",
          "rateToJpy must be a positive number",
        );
      }
      if (typeof currencyCode !== "string" || currencyCode.length !== 3) {
        throw new ApiErrorResponse(
          400,
          "VALIDATION_ERROR",
          "currencyCode must be a 3-character string",
        );
      }
