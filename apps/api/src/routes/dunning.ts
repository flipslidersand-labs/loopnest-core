  ...
  r.post("/", requireRole("admin"), async (req: any, res: any, next: any) => {
    try {
      ...
      if (!name || daysOverdue == null) {
        return res
          .status(400)
          .json({ error: "name and daysOverdue are required" });
      }
      if (typeof daysOverdue !== "number" || daysOverdue < 0) {
        return res
          .status(400)
          .json({ error: "daysOverdue must be a non-negative integer" });
      }
      ...
      const rule = await repos.dunning.createRule({
        name,
        daysOverdue,
        action,
        messageTemplate,
      ...
      if (err?.code === "23505")
        return res
          .status(409)
          .json({
            error: "A rule for this days_overdue + action already exists",
          });
      next(err);
    }
  });
  ...
  r.patch(
    ...
    async (req: any, res: any, next: any) => {
      try {
        ...
        const updated = await repos.dunning.updateRule(req.params.id, {
          name,
          messageTemplate,
          isActive,
        });
