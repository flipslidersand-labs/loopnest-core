...
export function quoteRoutes(repos: RepositoryContainer) {
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      let quotes, count;
      if (customerId) {
        quotes = await repos.quotes.findByCustomer(customerId, {
          skip,
          take,
        ...
        quotes = await repos.quotes.findByStatus(
          status as QuoteEntity["status"],
          { skip, take, organizationId: orgId },
        );
        count = await repos.quotes.count({ status, organizationId: orgId });
      } else {
        quotes = await repos.quotes.findAll({
          skip,
          take,
          organizationId: orgId,
        });
  ...
  router.get(
    "/:id",
    asyncHandler(async (req: Request, res: Response) => {
      const quote = await repos.quotes.findWithItems(
        req.params.id,
        req.user?.orgId,
      );
      if (!quote)
  ...
  router.patch(
    ...
    requireRole("editor", "admin"),
    asyncHandler(async (req: Request, res: Response) => {
      const { status, subtotalAmount, taxAmount, totalAmount, notes } =
        req.body;
  ...
  const requireDraftQuote = asyncHandler(
    async (req: Request, _res: Response, next: NextFunction) => {
      ...
      if (quote.status !== "draft") {
        throw new ApiErrorResponse(
          409,
          "INVALID_STATUS",
          "Quote must be in draft status to modify items",
        );
      }
      next();
    },
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (!productId || quantity == null || unitPrice == null) {
        throw new ApiErrorResponse(
          400,
          "VALIDATION_ERROR",
          "productId, quantity, and unitPrice are required",
        );
      }
      if (quantity <= 0)
        throw new ApiErrorResponse(
      ...
      const item = await repos.quoteItems.addItem(req.params.id, {
        productId,
        quantity,
        unitPrice,
      });
  ...
  router.patch(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const item = await repos.quoteItems.updateItem(
        req.params.itemId,
        req.params.id,
        { quantity, unitPrice },
      );
  ...
  router.delete(
    ...
    requireDraftQuote,
    asyncHandler(async (req: Request, res: Response) => {
      const removed = await repos.quoteItems.removeItem(
        req.params.itemId,
        req.params.id,
      );
      if (!removed)
        throw new ApiErrorResponse(404, "NOT_FOUND", "Item not found");
      const quote = await repos.quotes.findById(req.params.id);
      res.json({
        data: { success: true },
        quoteTotals: {
