...
export function invoicePaymentRoutes(
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      await audit.logPaymentRecorded(
        result.payment.id,
        invoiceId,
      ...
      if (result.balance.status === "paid") {
        await audit.logInvoiceMarkedPaid(
          invoiceId,
          req.user?.sub ?? "system",
          new Date(),
        );
...
export function paymentRoutes(
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (cursor || req.query.limit) {
        ...
        const page = await repos.payments.listPage({
          cursor,
          take,
          organizationId,
          invoiceId,
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      await audit.logPaymentReversed(
        id,
        result.balance.invoiceId,
        reason ?? "",
        req.user?.sub ?? "system",
      );
