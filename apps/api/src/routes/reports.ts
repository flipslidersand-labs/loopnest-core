...
export function reportRoutes(reportingService: ReportingService) {
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const summary = await reportingService.getMonthlySummary(
        month,
        req.user?.orgId,
      );
      res.json({ data: summary });
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const cashFlow = await reportingService.getCashFlow(
        from,
        to,
        req.user?.orgId,
      );
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const revenue = await reportingService.getRevenueByCustomer(
        month,
        req.user?.orgId,
      );
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (asOf && !/^\d{4}-\d{2}-\d{2}$/.test(asOf)) {
        throw new ApiErrorResponse(
          400,
          "VALIDATION_ERROR",
          "asOf must be an ISO date (YYYY-MM-DD)",
        );
      }
      const ar = await reportingService.getAccountsReceivable(
        req.user?.orgId,
        asOf,
      );
      res.json({ data: ar });
