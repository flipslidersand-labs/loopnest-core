...
export function customerRoutes(
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (cursor || req.query.limit) {
        ...
        const page = await repos.customers.findPage({
          cursor,
          take,
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      await audit.logResourceCreated(
        "customer",
        customer.id,
        req.user?.sub ?? "system",
        { name },
  ...
  router.patch(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      await audit.logResourceUpdated(
        "customer",
        req.params.id,
        req.user?.sub ?? "system",
        { name, address, phone },
  ...
  router.delete(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      await audit.logResourceDeleted(
        "customer",
        req.params.id,
        req.user?.sub ?? "system",
      );
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (isNaN(fromDate.getTime()) || isNaN(toDate.getTime())) {
        throw new ApiErrorResponse(
          400,
          "VALIDATION_ERROR",
          "from/to must be valid ISO date strings",
        );
      }
      ...
      if (creditLimit !== null && creditLimit !== undefined) {
        ...
        if (!Number.isFinite(val) || val < 0) {
          throw new ApiErrorResponse(
            400,
            "VALIDATION_ERROR",
            "creditLimit must be a non-negative number or null",
          );
        }
      }
