  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (orgId) {
        ...
        users = await repos.users.findByRole(role as User["role"], {
          skip,
          take,
        });
      ...
      res.json({
        ...
        filter: { role, organizationId: orgId },
      });
    }),
  );
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (tokenOrgId && existing.organizationId !== tokenOrgId) {
        throw new ApiErrorResponse(
          403,
          "FORBIDDEN",
          "You may only modify users in your organization",
        );
      }
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (tokenOrgId && existing.organizationId !== tokenOrgId) {
        throw new ApiErrorResponse(
          403,
          "FORBIDDEN",
          "You may only delete users in your organization",
        );
      }
