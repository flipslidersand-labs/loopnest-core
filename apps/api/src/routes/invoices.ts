...
const CSV_HEADER =
  "id,number,customer_id,amount,currency,status,created_at,due_date,paid_at";
...
export function invoiceRoutes(
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const take = Math.min(
        ...
        Math.max(
          1,
          Number.parseInt((req.query.limit ?? req.query.take) as string) || 20,
        ),
      );
      ...
      if (cursor || req.query.limit) {
        ...
        const page = await repos.invoices.findPage({
          cursor,
          take,
          status,
          customerId,
        ...
        const skip = Math.max(
          0,
          Number.parseInt(req.query.skip as string) || 0,
        );
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (!invoice)
        throw new ApiErrorResponse(404, "NOT_FOUND", "Invoice not found");
      const installments = await repos.installments.findByInvoice(
        req.params.id,
      );
      res.json({
        data: installments,
        meta: { count: installments.length, invoiceTotal: invoice.totalAmount },
      });
    }),
  );
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (invoice.status === "cancelled") {
        throw new ApiErrorResponse(
          409,
          "INVALID_STATUS",
          "Cannot schedule installments for a cancelled invoice",
        );
      }
      ...
      if (!Number.isInteger(n) || n < 2 || n > 24) {
        throw new ApiErrorResponse(
          400,
          "VALIDATION_ERROR",
          "count must be an integer between 2 and 24",
        );
      }
      if (!firstDueDate || !/^\d{4}-\d{2}-\d{2}$/.test(firstDueDate)) {
        throw new ApiErrorResponse(
          400,
          "VALIDATION_ERROR",
          "firstDueDate must be YYYY-MM-DD",
        );
      }
      ...
      if (!Number.isFinite(ivDays) || ivDays < 1 || ivDays > 365) {
        throw new ApiErrorResponse(
          400,
          "VALIDATION_ERROR",
          "intervalDays must be between 1 and 365",
        );
      }
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (!inst)
        throw new ApiErrorResponse(
          404,
          "NOT_FOUND",
          "Installment not found or already paid/cancelled",
        );
      if (inst.invoiceId !== req.params.id)
        throw new ApiErrorResponse(404, "NOT_FOUND", "Installment not found");
      res.json({ data: inst });
    }),
  );
  ...
  router.patch(
    ...
    requireRole("admin"),
    asyncHandler(async (req: Request, res: Response) => {
      const inst = await repos.installments.cancelInstallment(
        req.params.instId,
      );
      if (!inst)
        throw new ApiErrorResponse(
          404,
          "NOT_FOUND",
          "Installment not found or not pending",
        );
      if (inst.invoiceId !== req.params.id)
        throw new ApiErrorResponse(404, "NOT_FOUND", "Installment not found");
      res.json({ data: inst });
