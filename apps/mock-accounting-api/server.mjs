...
const server = createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/api/exports") {
    ...
    const failByName =
      FAIL_INVOICE_SUBSTR &&
      String(body.invoiceNumber || "").includes(FAIL_INVOICE_SUBSTR);
    ...
    console.log(
      `[mock-accounting] ACCEPT export ${record.invoiceNumber} -> ${record.exportId}`,
    );
...
server.listen(PORT, () => {
  ...
  if (FAIL_INVOICE_SUBSTR)
    console.log(`   failing invoices containing "${FAIL_INVOICE_SUBSTR}"`);
  if (failTimesRemaining)
    console.log(`   failing first ${failTimesRemaining} attempts`);
});
